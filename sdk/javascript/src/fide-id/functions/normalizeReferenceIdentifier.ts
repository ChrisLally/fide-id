...
} from "./expandPredicateReferenceIdentifier.js";
import type {
...
/**
 * Normalize and validate a predicate reference identifier.
 *
 ...
 * @throws Error if referenceIdentifier is not a valid canonical predicate URL.
 */
export function normalizePredicateReferenceIdentifier(
  ...
  const candidate = options?.expandPrefixes
    ? expandPredicateReferenceIdentifier(referenceIdentifier, { prefixes: options.prefixes })
    : referenceIdentifier;
  ...
  const normalizedCandidate = options?.expandPrefixes
    ? normalizeReferenceIdentifier(candidate, { skipUrlNormalization })
    : normalized;
